...
#define SPISPEED 1024000 //
//#define SPISPEED  512000 // TEMP bits: 440(60C) - 480(90C) , 12.2V , 1uF
//#define SPISPEED 196608 // 192 000 Hz is taken
...
static char          chipbank[MAXCHIPS+1]; // bank of the chip (in version 2 chips are on cards on 4 banks (4 cards per bank)
...
int version=0; // M-board version detection and protection
...
void spi_init(void)
{
	int mode=0,bits=8,maxspeed=4000000; // 24 000 000;
	//int i,mode=0,bits=8,maxspeed=1000000; // 24 000 000;
	//int i,mode=0,bits=8,maxspeed= 256000; // 24 000 000;
...
{
	const int banks[9]={18,23,24,25,8,7, 17,27,22}; // GPIO connected to multiplexers, 
	//const int banks[5]={18,23,24,8,7}; // GPIO connected to multiplexers
	//const int map[MAXBANKS]={5,7,6,4,3,0,1,2};
	//const int map[MAXBANKS]={5,7,6,4,2,1,0,3};
	//const int map[MAXBANKS]={5,4,6,7,2,1,0,3};
	const int map[MAXBANKS]={5,7,6,4,3,0,1,2};
	int i,k;
	//unsigned int pin;

	assert(MAXBANKS==48);
        assert(bank>=1 && bank<=MAXBANKS);
	INP_GPIO(10); OUT_GPIO(10);
	INP_GPIO(11); OUT_GPIO(11);
	for(i=0;i<6;i++){
		if((bank-1)/8==i){
			GPIO_CLR = 1 << banks[i];} // enable multiplexers, active low !
		else{
			GPIO_SET = 1 << banks[i];}} // disable multiplexers
        for(i=0;i<3;i++){
                if(map[(bank-1)%8]>>i & 0x1){
                        GPIO_SET = 1 << banks[6+i];} // enable pin
                else{
                        GPIO_CLR = 1 << banks[6+i];}} // disable pin
	usleep(1);
	...
	//usleep(16);
	//pin=GPIO_LEV;
	//fprintf(stderr,"reset bank %d [%d] (%08x,%d)\n",bank,max,pin,(pin&(1<<9)?1:0));
}
void spi_ledon(void)
{	GPIO_CLR = 1 << 16; // LED ON
}
void spi_ledoff(void)
{	GPIO_SET = 1 << 16; // LED OFF
}
int spi_check_version(void)
{
	// there is only one version now
	// GPIO25 is used for testing
	//const int banks[5]={18,23,24,8,7}; // GPIO connected to multiplexers, 
	const int banks[9]={18,23,24,25,8,7, 17,27,22}; // GPIO connected to multiplexers, 
	int i;
	unsigned int pin;
        //INP_GPIO(14);
        //INP_GPIO(15); // pulled up with 50kOhm anyway :-(
        INP_GPIO(7);
	usleep(10);
	pin=GPIO_LEV;
	if(!(pin&(1<<7))){
		fprintf(stderr,"ERROR: GPIO7 low, fatal (%08x)\n",pin);
		exit(-1);}
        for(i=0;i<9;i++){
                INP_GPIO(banks[i]);
                OUT_GPIO(banks[i]);}
	...
	pin=GPIO_LEV;
	if(pin&(1<<15)){
		fprintf(stderr,"ERROR: GPIO15 high, fatal (%08x)\n",pin);
		exit(-1);}
	GPIO_SET = 1 << 7;
	usleep(10);
	pin=GPIO_LEV;
	if(!(pin&(1<<15))){
		fprintf(stderr,"ERROR: GPIO15 low, fatal (%08x)\n",pin);
		return(4);
		exit(-1);}*/
	return(5);
}
...
int spi_txrx(const char *wrbuf,char *rdbuf,int bufsz,const unsigned int* off,const unsigned int* boff)
{
	...
	int len=bufsz;
#endif
	int i=0,bank,reset=1;
	unsigned int pos=0;
	struct spi_ioc_transfer tr;
	memset(&tr,0,sizeof(tr));
	tr.delay_usecs = 1; // was 1
...
*/
	...
	for(bank=1;bank<=MAXBANKS;bank++){
		if(boff[bank]){
			//spi_reset(64,bank);
			spi_reset(8,bank);
			break;}}
	assert(bank<=MAXBANKS);
        for(;bufsz>0;bufsz-=tr.len,wrbuf+=tr.len,rdbuf+=tr.len,pos+=tr.len,reset=0) {
		...
		for(;i<=maxchips;i++){
			if(!off[i]){
				continue;}
			if(off[i]>=pos+tr.len){
				if(!reset || chipspis[i]<tr.speed_hz){ // do first block slow so that all chips in the chain can read the emit_break command
					if(chipspis[i]!=SPISPEED && chipspis[i]!=512000){
						fprintf(stderr,"chip: %d , spis: %d\n",i,chipspis[i]);}
					tr.speed_hz=chipspis[i];}
				break;}}
		assert(i<=maxchips);
		//assert(chipspis[i]!=SPISPEED);
		assert(tr.speed_hz==SPISPEED || tr.speed_hz==512000);
		//assert(reset || tr.speed_hz!=SPISPEED);
...
{
	spmut.lock();
	if(spibufst[0]==SPIBUF_DONE || spibufst[1]==SPIBUF_DONE){
		spmut.unlock();
		return 1;}
	spmut.unlock();
	return 0;
...
int spi_put(hasht mids,datat data,datat* avec,char* chipconf,char* chipfast)
{
	...
	for(i=0;i<maxchips;i++){ // program all chips
		int update=0;
		//int newbank=0; does not help
		if(chipbank[i]!=bank){
			//newbank=1; does not help
			assert(bankoff[spinow][bank]==0);
			spi_emit_buf("\x0",1);
			bankoff[spinow][bank]=spibufsz[spinow];
			bank=chipbank[i];
			spi_emit_break();}
		if(chipconf[i]&0x80){ // this is a temp sensor
			chipoff[spinow][i]=spibufsz[spinow]; // remember position with results
			spi_emit_buf((const char*)avec[i],19*4);
			continue;} // should try a shorter buffer
		//if(oldfast[i]!=chipfast[i] || i==fixchip || (chipconf[i] & 0x40)){
		if(oldfast[i]!=chipfast[i] || (i==fixchip && (chipconf[i] & 0x40))){
			update++;
			//printf("SPI for chip %d: change speed\n",i);
			spi_emit_data(0x6000,(const char*)chip_osc((int)chipfast[i]),8);
			oldfast[i]=chipfast[i];}
		//if(oldconf[i]!=chipconf[i] || i==fixchip || (chipconf[i] & 0x40)){
		if(i==fixchip && (chipconf[i] & 0x40)){
			update++;
		...
		if((oldconf[i]&0x3E)!=(chipconf[i]&0x3E)){
			update++;
		...
		if(i==fixchip && (chipconf[i] & 0x40)){
			...
			chipconf[i] &= ~0x40;
			oldconf[i]=chipconf[i];}
		if(update && fixchip!=MAXCHIPS){
			printf("chip %d programmed\n",i+1);}
		//if(newbank){ // on version 2 boards send data to first chip 2 times to boost level shifter settings
                //	spi_emit_data(0x3000,(const char*)avec[i],19*4);}
		chipoff[spinow][i]=spibufsz[spinow]+3; // remember position with results
		if(chipconf[i]&0x3E){ // or completely ignore chip
			spi_emit_data(0x3000,(const char*)avec[i],19*4);}
		spi_emit_fasync();}
	spi_emit_buf("\x0",1);
...
int spi_get(hasht mids,datat data,datat* rvec,char* chipconf)
{
	...
	for(i=0;i<maxchips;i++){
		if(chipconf[i] & 0xBE){ // if not completely ignored
...
#ifdef USEALL
	maxchips=last;
        for(i=first;i<last && i<MAXCHIPS;i++){
...
int spi_start(char* chipconf,char* chipfast,char* chipcard)
{
	int b=1,c,lb=0;
	...
	spi_init();
	version=spi_check_version();
	if(version!=5){
		printf("M-Board version 5 detected, not detected, fatal\n");
		exit(-1);}
	for(b=1;b<=MAXBANKS;b++){
		int c=maxchips,max=maxchips+BANKCHIPS;
		for(;c<=max && c<=MAXCHIPS;c++){
			chipspis[c]=512000;}
		spi_reset(128,b);
		spi_programm(oldconf,oldfast,b,maxchips,maxchips+BANKCHIPS);}
	printf("M-Board version 5 detected\n");
	for(c=0;c<maxchips;c++){
		chipspis[c]=SPISPEED;
		if(!chipbank[c]){
			chipcard[c]=c/BOARDCHIPS;
			continue;}
		if(chipbank[c]!=chipbank[lb]){
			lb=c;}
		chipcard[c]=(BANKCHIPS/BOARDCHIPS)*(chipbank[lb]-1)+(c-lb)/BOARDCHIPS;}
	chipspis[c]=SPISPEED;
        spt->Start();
	return(maxchips);
}
...
{	int i,talk,wait;
	...
	while(!testCancel()) {
		...
		wait=1000000*(stop.tv_sec-start.tv_sec)+stop.tv_usec-start.tv_usec;
		if(wait<500000){
			printf("SPI SLEEPING %f msec\n",(500000-wait)/1000.0);
			threads_sleep((500000-wait)/1000);}
		if(wait>1000000){
	...
	if(!version){
		return;}
	memset(oldconf,0x02,sizeof(oldfast));
	memset(oldfast,0,sizeof(oldfast));
	/*spi_reset(1024,0);
	maxchips=0;
	if(version==1){
		spi_programm(oldconf,oldfast,0,0,MAXCHIPS);}
	else{
		b=1;
		for(;b<=MAXBANKS;b++){
			spi_reset(1024,b);
